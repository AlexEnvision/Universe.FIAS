...
using Universe.Fias.DBF.Converter.x64.Algorithms.AddressSystem.Conversion.DBF;
...
namespace Universe.Fias.DBF.Converter.x64.Algorithms.AddressSystem.Conversion
{
    public partial class AddrSysConversionManager
    {
        ...
        public class FileTaskconversion
        {
            ...
            public async Task RunAsync(EventLogger logStep)
            {
                await Task.Factory.StartNew(
                    () =>
                    {
                        ...
                        var filePath = Path.Combine(_conversionContext.FiasDbfFolderPath, File);

                        // Перобразование файла DBF в файл CSV
                        DbfConversionThread(logStep, filePath);
            ...
            private void DbfConversion(EventLogger logStep, string filePath)
            {
                var csvfileName = File.Replace(".dbf", ".csv", StringComparison.InvariantCultureIgnoreCase);
                logStep.Info($"Convert {File} to {csvfileName}");

                string dbfTableName = Path.GetFileNameWithoutExtension(File);

                var factory = new DbfConversionTasksFactory(_conversionContext, File);
                factory.CreateExecTask(dbfTableName);
            }

            private void DbfConversionThread(EventLogger logStep, string filePath)
            {
                ...
                var thread = new Thread(
                    state => {
                        try
                        {
                            DbfConversion(logStep, filePath);
                        }
                        catch (Exception ex)
                        {
                            error = ex;
                ...
                if (error != null)
                    throw new Exception($"Dbfconversion error: {error.Message}", error);
            }
            ...
            private void ExtractFileFromArchive(EventLogger logStep)
            {
                ...
                sevenZipArchive.UnpackArchive(
                    _conversionContext.FiasDbfArchivePath,
                    _conversionContext.FiasDbfFolderPath,
