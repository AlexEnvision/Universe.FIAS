...
using Universe.Fias.Normalizer.Npg.ConsoleApp.Tests.Base;

namespace Universe.Fias.Normalizer.Npg.ConsoleApp.Tests
{
    internal class NormalizerTest : BaseTest
    {
        ...
        public NormalizerTest(IUniverseLogger log) : base(log)
        {
            try
            {
                ...
                settings.ConnectionString =
                    "Server=192.168.2.180;Database=FIAS-DB;Port=5432;UserId=postgres;Password=*********;Integrated Security=false;CommandTimeout=3600";

                UniverseNpgDbCache.ConnectionString = settings.ConnectionString;
                MigrateDb(settings);

                var resolver = new AppPrincipalResolver();
                var scope = new UniverseFiasScope(resolver, settings, container, DbSystemManagementTypes.PostgreSQL);
        ...
        private void MigrateDb(AppSettings settings)
        {
            try
            {
                Log.Info("Запуск миграции модельной БД...");
                using (var dbContext = new UniverseFiasNpgDbContext(settings.ConnectionString)
                ) //TODO -> Добавить фабрику контекстов
                {
                    dbContext.Migrate();
                }

                Log.Info("Успешно завершена миграция модельной БД!");

                Log.Info("Запуск \"контрольной\" миграции модельной БД...");
                using (var dbContext = new UniverseFiasNpgDbContext(settings.ConnectionString)
                ) //TODO -> Добавить фабрику контекстов
                {
                    dbContext.Migrate();
                ...
                Log.Info("Успешно завершена \"контрольная\" миграция модельной БД!");
            }
            catch (Exception ex)
            {
                Log.Error(ex, ex.Message);
