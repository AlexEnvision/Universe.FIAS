...
namespace Universe.Fias.Import.Algorithms.AddressSystem
{
    ...
    public class AsVersionsBOper
    {
        ...
        /// <summary>
        /// Gets the last imported version.
        /// </summary>
        /// <returns></returns>
        public AsVersion GetLastImportedVersion()
        {
        ...
        /// <summary>
        /// Gets the last imported version Full.
        /// </summary>
        /// <returns></returns>
        public AsVersion GetLastImportedVersionFullDbf()
        {
            var asVersion = _db.AsVersions.Where(_ => _.IsProcesed == true && _.IsDownloadedFiasFullDbf == true)
                .OrderByDescending(_ => _.VersionId).FirstOrDefault();
            return asVersion;
        }
        ...
        public List<AsVersion> GetVersionsForDownload(int? versionId)
        {
            var query = _db.AsVersions
                .Where(_ => _.IsVersionFromFiasService && !(_.IsDownloadedFiasFullDbf == true || _.IsDownloadedFiasDeltaDbf == true));
            if (versionId != null)
        ...
        /// <summary>
        /// Gets the versions for import.
        /// </summary>
        /// <returns></returns>
        public List<AsVersion> GetVersionsForImport()
        {
            var query = _db.AsVersions
                .Where(_ => _.IsProcesed.Value == false);
        ...
        public void UpdateVersionEndDownload(long id, [NotNull] string filePath)
        {
            ...
            if (version.IsDownloadedFiasFullDbf != null)
            {
                version.IsDownloadedFiasFullDbf = true;
                version.IsDownloadedFiasDeltaDbf = null;
            }
            else if (version.IsDownloadedFiasDeltaDbf != null)
            {
                version.IsDownloadedFiasFullDbf = null;
                version.IsDownloadedFiasDeltaDbf = true;
            }
            else
            {
                throw new NotSupportedException($"Unknown type download (IsDownloadedFiasFullDbf or IsDownloadedFiasDeltaDbf)");
            }

            version.EndDateDownload = DateTimeOffset.Now;
            version.FiasDbfArchivePath = filePath;
        ...
        public void UpdateVersionStartDownload(AsVersion version, bool isFullDbf)
        {
            if (isFullDbf)
            {
                version.IsDownloadedFiasFullDbf = false;
                version.IsDownloadedFiasDeltaDbf = null;
            }
            else
            {
                version.IsDownloadedFiasFullDbf = null;
                version.IsDownloadedFiasDeltaDbf = false;
            }
