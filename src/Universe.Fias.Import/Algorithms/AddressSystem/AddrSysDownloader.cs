    ...
    public class AddrSysDownloader : DisposableObject
    {
        ...
        public void Download()
        {
            _scope.UniverseFiasDb(
                db => {
                    var oper = new AsVersionsBOper(db);
                    var lastProcessedVersion = oper.GetLastImportedVersion();
                    //logStep.Stat.LastProcessedVersion = lastProcessedVersion;
                    var versionsForDownload = oper.GetVersionsForDownload(lastProcessedVersion?.VersionId);

                    //logStep.Stat.VersionsForDownload = versionsForDownload;
                    ...
                    var lastProcessedVersionFull = oper.GetLastImportedVersionFullDbf();
        ...
        private async Task StartDownloadAsync(AsVersion versionForDownload)
        {
            ...
            try
            {
                ...
                //_logger.Stat.UrlForDownload = urlForDownload;

                var filePath = _folderManager.GetFilePathByVersion(
                    versionForDownload,
                    _folderManager.GetFileNameFromUrl(urlForDownload));

                //_logger.Stat.FilePath = filePath;
            ...
            catch (Exception ex)
            {
                ...
                //_logger.LogError($"Error download {_logger.Stat.UrlForDownload} to {_logger.Stat.FilePath}: {ex.Message}", ex);
        ...
        //private void DownloadProgressChanged(ILogStepScope<AddrSysDownloadVersionStat> logStep, DownloadProgressChangedEventArgs args)
        //{
        //    lock (logStep.Stat)
        //        if (args.TotalBytesToReceive == args.BytesReceived
        //            || args.ProgressPercentage != logStep.Stat.ProgressPercentage)
        //        {
        //            logStep.Stat.TotalBytesToReceive = args.TotalBytesToReceive;
        //            logStep.Stat.BytesReceived = args.BytesReceived;
        //            logStep.Stat.ProgressPercentage = args.ProgressPercentage;
        //            logStep.UpdateStat();
        //        }
        //}
    }
