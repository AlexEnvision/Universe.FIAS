...
namespace Universe.Fias.Core.Infrastructure.SubScopes
{
    ...
    /// </summary>
    internal class UniverseFiasNpgSubScope : UniverseScope<UniverseFiasNpgDbContext>, IUniverseFiasSubScope
    {
        ...
        public UniverseFiasNpgSubScope(UniverseFiasScope mainScope, IWebAppPrincipalResolver principalResolver, IAppSettings appSettings, IUnityContainer container) : base(appSettings, container)
        {
            if (container == null) 
        ...
        public void UniverseFiasDb(Action<IUniverseFiasDbContext> action)
        {
            ...
            using (var db = new UniverseFiasNpgDbContext(DbCtx.Database.Connection.ConnectionString))
                action(db);
        }
